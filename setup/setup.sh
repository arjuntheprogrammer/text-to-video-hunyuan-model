...
upsert_env_key "${ENV_FILE}" "HF_HOME" "./models"
upsert_env_key "${ENV_FILE}" "HF_HUB_CACHE" "./models"
remove_env_key "${ENV_FILE}" "HF_HUB_ENABLE_HF_TRANSFER"
upsert_env_key "${ENV_FILE}" "HF_XET_HIGH_PERFORMANCE" "1"
upsert_env_key "${ENV_FILE}" "LOCAL_FILES_ONLY" "1"
upsert_env_key "${ENV_FILE}" "ALLOW_REMOTE_FALLBACK" "1"
# TRANSFORMERS_CACHE is deprecated in transformers v5; remove to avoid warnings.
remove_env_key "${ENV_FILE}" "TRANSFORMERS_CACHE"
upsert_env_key "${ENV_FILE}" "TORCH_HOME" "./models"
upsert_env_key "${ENV_FILE}" "OUTPUT_DIR" "./outputs"
upsert_env_key "${ENV_FILE}" "LOG_DIR" "./logs"
upsert_env_key "${ENV_FILE}" "APP_LOG" "./logs/hunyuan_app.log"
upsert_env_key "${ENV_FILE}" "PROGRESS_LOG_EVERY_STEPS" "1"
upsert_env_key "${ENV_FILE}" "PROGRESS_BAR_WIDTH" "24"
upsert_env_key "${ENV_FILE}" "AUTO_MAX_INPUT_SIDE" "1"
current_max_input_side="$(read_env_key "${ENV_FILE}" "MAX_INPUT_IMAGE_SIDE")"
if [[ -z "${current_max_input_side}" ]]; then
  upsert_env_key "${ENV_FILE}" "MAX_INPUT_IMAGE_SIDE" "auto"
fi
upsert_env_key "${ENV_FILE}" "OOM_SAFE_NUM_FRAMES" "32"
upsert_env_key "${ENV_FILE}" "OOM_SAFE_STEPS" "12"
upsert_env_key "${ENV_FILE}" "ENABLE_SEQUENTIAL_CPU_OFFLOAD" "0"
upsert_env_key "${ENV_FILE}" "ENABLE_MODEL_CPU_OFFLOAD" "1"
upsert_env_key "${ENV_FILE}" "DEFAULT_FPS" "16"
upsert_env_key "${ENV_FILE}" "DEFAULT_DURATION_SECONDS" "10"
upsert_env_key "${ENV_FILE}" "DEFAULT_NUM_INFERENCE_STEPS" "45"
upsert_env_key "${ENV_FILE}" "DEFAULT_GUIDANCE_SCALE" "7.5"
upsert_env_key "${ENV_FILE}" "MAX_PROMPT_WORDS" "60"
upsert_env_key "${ENV_FILE}" "MAX_NEGATIVE_PROMPT_WORDS" "60"
upsert_env_key "${ENV_FILE}" "DEFAULT_OUTPUT_LONG_EDGE" "1080"
upsert_env_key "${ENV_FILE}" "OUTPUT_LONG_EDGE_OPTIONS" "720,1080,1440"
upsert_env_key "${ENV_FILE}" "ENABLE_DEFLICKER" "0"
upsert_env_key "${ENV_FILE}" "DEFLICKER_WINDOW" "3"
upsert_env_key "${ENV_FILE}" "QUALITY_PROFILE" "balanced"
upsert_env_key "${ENV_FILE}" "MAX_FRAMES_BY_PROFILE" "low:160,balanced:320,high:320"
upsert_env_key "${ENV_FILE}" "MAX_STEPS_BY_PROFILE" "low:20,balanced:28,high:32"
upsert_env_key "${ENV_FILE}" "MAX_INPUT_SIDE_BY_PROFILE" "low:768,balanced:1024,high:1280"
upsert_env_key "${ENV_FILE}" "ENABLE_SHARPEN" "0"
upsert_env_key "${ENV_FILE}" "SHARPEN_STRENGTH" "0.35"
upsert_env_key "${ENV_FILE}" "ENABLE_CAPTIONING" "1"
upsert_env_key "${ENV_FILE}" "CAPTION_MODEL_ID" "nlpconnect/vit-gpt2-image-captioning"
upsert_env_key "${ENV_FILE}" "CAPTION_DEVICE" "cuda"
upsert_env_key "${ENV_FILE}" "CAPTION_MAX_TOKENS" "40"
upsert_env_key "${ENV_FILE}" "CAPTION_UNLOAD_AFTER_USE" "1"
